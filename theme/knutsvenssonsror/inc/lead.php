 ...
 * or photo. They call kn_lead() instead. To build a demo for a real firm, the
 ...
 *
 * With no lead-data.php present the defaults below render the fictional Knut Svenssons Rör AB.
 *
 ...
 * @return string
 */
function kn_sample_note() {
	return kn_lead( 'ar_riktig' ) ? '' : ' <span class="kn-sample">(platshållare)</span>';
}
 ...
 * @return string File name for kn_lead_img().
 */
function kn_lead_bild( $i ) {
	$bilder = (array) kn_lead( 'bilder', array() );
 ...
 * @return string
 */
function kn_ring_label() {
	$namn = kn_lead( 'kontakt' );
	return $namn ? sprintf( 'Ring %s · %s', $namn, kn_lead( 'telefon' ) ) : 'Ring ' . kn_lead( 'telefon' );
}
