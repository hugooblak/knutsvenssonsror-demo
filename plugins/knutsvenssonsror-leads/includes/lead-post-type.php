...
add_action(
	...
	function () {
		register_post_type(
			...
			array(
				'labels'          => array(
					'name'          => __( 'Leads', 'knutsvenssonsror-leads' ),
					'singular_name' => __( 'Lead', 'knutsvenssonsror-leads' ),
					'edit_item'     => __( 'Förfrågan', 'knutsvenssonsror-leads' ),
					'search_items'  => __( 'Search leads', 'knutsvenssonsror-leads' ),
					'not_found'     => __( 'Inga förfrågningar än. Skicka formuläret så dyker den upp här.', 'knutsvenssonsror-leads' ),
					'all_items'     => __( 'Alla förfrågningar', 'knutsvenssonsror-leads' ),
				),
...
add_filter(
	...
	function () {
		return array(
			...
			'title'        => __( 'Name', 'knutsvenssonsror-leads' ),
			'npl_phone'    => __( 'Phone', 'knutsvenssonsror-leads' ),
			'npl_service'  => __( 'Needs', 'knutsvenssonsror-leads' ),
			'npl_timeline' => __( 'When', 'knutsvenssonsror-leads' ),
			'npl_zip'      => __( 'ZIP', 'knutsvenssonsror-leads' ),
			'npl_source'   => __( 'Source', 'knutsvenssonsror-leads' ),
			'npl_status'   => __( 'Status', 'knutsvenssonsror-leads' ),
			'date'         => __( 'Received', 'knutsvenssonsror-leads' ),
		);
	}
);
...
function npl_source_summary( $post_id ) {
	...
	return __( 'Direct', 'knutsvenssonsror-leads' );
}
...
add_action(
	...
	function () {
		add_meta_box( 'npl_details', __( 'What they told us', 'knutsvenssonsror-leads' ), 'npl_details_box', 'npl_lead', 'normal', 'high' );
		add_meta_box( 'npl_status', __( 'Status', 'knutsvenssonsror-leads' ), 'npl_status_box', 'npl_lead', 'side', 'high' );
	}
);
...
function npl_details_box( $post ) {
	$rows = array(
		__( 'Name', 'knutsvenssonsror-leads' )     => get_post_meta( $post->ID, 'name', true ),
		__( 'Phone', 'knutsvenssonsror-leads' )    => get_post_meta( $post->ID, 'phone', true ),
		__( 'E-post', 'knutsvenssonsror-leads' )    => get_post_meta( $post->ID, 'email', true ),
		__( 'ZIP', 'knutsvenssonsror-leads' )      => get_post_meta( $post->ID, 'zip', true ),
		__( 'Needs', 'knutsvenssonsror-leads' )    => npl_label( 'service', get_post_meta( $post->ID, 'service', true ) ),
		__( 'When', 'knutsvenssonsror-leads' )     => npl_label( 'timeline', get_post_meta( $post->ID, 'timeline', true ) ),
		__( 'Source', 'knutsvenssonsror-leads' )   => npl_source_summary( $post->ID ),
	);
	...
	$rows[ __( 'Samtycke lämnat', 'knutsvenssonsror-leads' ) ] = get_post_meta( $post->ID, 'consent_at', true ) . ' — "' . get_post_meta( $post->ID, 'consent_text', true ) . '"';
...
function npl_status_box( $post ) {
	...
	echo '<label class="screen-reader-text" for="npl-status">' . esc_html__( 'Status', 'knutsvenssonsror-leads' ) . '</label><select id="npl-status" name="npl_status" style="width:100%">';
