...
add_filter(
	...
	function ( $exporters ) {
		$exporters['knutsvenssonsror-leads'] = array(
			'exporter_friendly_name' => __( 'Quote requests', 'knutsvenssonsror-leads' ),
			'callback'               => function ( $email ) {
				$items = array();
				foreach ( npl_leads_by_email( $email ) as $lead ) {
					$data = array( array( 'name' => __( 'Received', 'knutsvenssonsror-leads' ), 'value' => $lead->post_date ) );
					...
					$items[] = array(
						'group_id'    => 'knutsvenssonsror-leads',
						'group_label' => __( 'Quote requests', 'knutsvenssonsror-leads' ),
...
add_filter(
	...
	function ( $erasers ) {
		$erasers['knutsvenssonsror-leads'] = array(
			'eraser_friendly_name' => __( 'Quote requests', 'knutsvenssonsror-leads' ),
			'callback'             => function ( $email ) {
				$removed = false;
				foreach ( npl_leads_by_email( $email ) as $lead ) {
